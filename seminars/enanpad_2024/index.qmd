...
Collider bias occurs when an independent variable and outcome each influence a common third variable and that variable or collider is controlled for in the regression.

    - In a way, a collider is a bad control. 
    - While a bad control makes you underestimate the coefficient, a collider creates spurious correlation between the X and the Y.
    
In the analysis below

    - $Q_{i} = \alpha + \beta × Gov_{i} + Controls + \epsilon$
    
Including, for instance, CEO Reputation (assuming that both $Q$ and $Gov$ influences Reputation) creates a false correlation between $Qov$ and $Q$.

    - You may estimate a positive association even when there is none.
...
## 9) Heterogeneous effects  {.smaller }
...
## 10) Measurement error   {.smaller }

- "Classical" random measurement error in x’s will bias  **coefficient toward zero**
