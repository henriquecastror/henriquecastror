...
(function(){
  ...
  const BASE = REPORTS_BASE; // já definido no topo da página

  // ---------- Coleta do student_id ----------
  const norm = s => {
    const r = String(s||"").trim().toLowerCase();
    return r.includes("@") ? r.split("@")[0] : r;
  };
  const looksLikeId = v => /^[a-z0-9._-]{2,}$/i.test(v || "");

  function fromQuery(){
    const p = new URLSearchParams(location.search);
    const v = norm(p.get("me") || p.get("sid") || "");
    return looksLikeId(v) ? v : "";
  }
  function fromStorage(){
    try{
      const keys = [
        "student_login","vhcm_current_user","hcm_current_user",
        "current_user","user_login","login","user","sid","student"
      ];
      for (const k of keys){
        const v = norm(localStorage.getItem(k) || sessionStorage.getItem(k) || "");
        if (looksLikeId(v)) return v;
      }
      // heurística: varre tudo
      for (const S of [localStorage, sessionStorage]){
        for (let i=0;i<S.length;i++){
          const val = norm(S.getItem(S.key(i)));
          if (looksLikeId(val)) return val;
          if (/^[^@\s]+@[^@\s]+$/.test(val)) return norm(val);
        }
      }
    }catch{}
    // possíveis variáveis globais do seu gate
    try{
      if (looksLikeId(window.CURRENT_STUDENT)) return norm(window.CURRENT_STUDENT);
      if (looksLikeId(window.__LOGIN?.studentId)) return norm(window.__LOGIN.studentId);
    }catch{}
    return "";
  }
  function fromCookies(){
    try{
      const bag = Object.fromEntries(document.cookie.split(/; */).filter(Boolean).map(kv=>{
        const i = kv.indexOf("="); return [decodeURIComponent(kv.slice(0,i)), decodeURIComponent(kv.slice(i+1))];
      }));
      const v = norm(bag.student_id || bag.sid || bag.hcm_student || "");
      if (looksLikeId(v)) return v;
      const email = (document.cookie.match(/(?:^|;)\s*[^=]+=([^;@]+@[^;]+)/)||[])[1];
      if (email) return norm(email);
    }catch{}
    return "";
  }
  function getStudent(){ return fromQuery() || fromStorage() || fromCookies() || ""; }

  // ---------- Ação do botão ----------
  async function openMyPdf(){
    const sid = getStudent();
    if (!sid){ alert("Faça login primeiro para gerar seu PDF."); return; }
    ...
    // Abre a aba antes do fetch para evitar bloqueio
    ...
    try{
      ...
      win.location = url;                  // carrega o PDF na nova aba
